...
#if defined(NDEBUG)
# error "FirstByteCoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x36cce12c3bae13297b419acdd47e848337c7b750f040b6429e1c2a83349dad75");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // FirstByteCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "FirstByteCoin Signed Message:\n";
...
{
    // FirstByteCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
if(nHeight == 1)   
    ...
    if (nHeight != 0)
	{
        for (int frr = 0; frr < nHeight /30; frr++)
		{
...
static const int64 nTargetTimespan = 4 * 60; // FirstByteCoin: 4 minutes
static const int64 nTargetSpacing = 2 * 60; // FirstByteCoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // FirstByteCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // FirstByteCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x36cce12c3bae13297b419acdd47e848337c7b750f040b6429e1c2a83349dad75");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "25.08.2017 FirstByte ist FIRST";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042174710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1503606463;
        ...
        assert(block.hashMerkleRoot == uint256("0x244c7710be9a968cba29f7b6dd8c6ae78ebb7ae8e465a7860f077bf2cafc4117"));
...
unsigned char pchMessageStart[4] = { 0xba, 0xa4, 0xb3, 0x9b }; // FirstByteCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FirstByteCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FirstByteCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FirstByteCoinMiner(CWallet *pwallet)
{
    printf("FirstByteCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("firstbytecoin-miner");
    ...
    try { loop {
        ...
        printf("Running FirstByteCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FirstByteCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FirstByteCoinMiner, pwallet));
}
